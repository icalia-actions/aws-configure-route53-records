...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        const comment = core_1.getInput("comment");
        const records = core_1.getInput("records");
        const hostedZoneId = core_1.getInput("hosted-zone-id");
        const changeInfo = yield dns_record_management_1.configureDnsRecordSet({
            comment,
            records,
            hostedZoneId,
        });
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.configureDnsRecordSet = void 0;
...
function parseRecordSet(jsonRecordSet) {
    // parseYaml takes care of both YAML and JSON strings
    return yaml_1.parse(jsonRecordSet || "null");
}
function readRecordSetFile(filePath) {
    const templateContents = fs.readFileSync(filePath, "utf8");
    return parseRecordSet(templateContents);
}
function processRecordSetInput(recordInput) {
    let records;
    if (!fs.existsSync(recordInput))
        records = parseRecordSet(recordInput);
    else
        records = readRecordSetFile(recordInput);
    return records.map((record) => {
        return { Action: "UPSERT", ResourceRecordSet: record };
    });
}
function configureDnsRecordSet(input) {
    return __awaiter(this, void 0, void 0, function* () {
        const { hostedZoneId, records, comment } = input;
        const changes = processRecordSetInput(records);
        const client = getClient();
        const { ChangeInfo } = yield client
...
exports.configureDnsRecordSet = configureDnsRecordSet;
